...
sshpass -p "$VPS_PASS" ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o PreferredAuthentications=password -o PubkeyAuthentication=no root@$VPS_IP "echo 'password=$TEST_USER_PASS' > /root/.vps_secrets && chmod 600 /root/.vps_secrets"

echo "Executing setup.sh..."
sshpass -p "$VPS_PASS" ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o PreferredAuthentications=password -o PubkeyAuthentication=no root@$VPS_IP "export LC_ALL=en_US.UTF-8 && cd vps-rdp-workstation && VPS_USERNAME=$TEST_USER VPS_SECRETS_FILE=/root/.vps_secrets ./setup.sh --ci" || echo "Setup finished or disconnected (expected if root login disabled)"

echo "=== 6. Verify Installation (User Access) ==="
echo "Waiting for SSH to reload..."
sleep 15
echo "Attempting login as $TEST_USER..."
sshpass -p "$TEST_USER_PASS" ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o PreferredAuthentications=password -o PubkeyAuthentication=no $TEST_USER@$VPS_IP "echo '✓ User login successful'; hostname; echo 'Checking Docker group...'; groups | grep docker && echo '✓ Docker group present'"
