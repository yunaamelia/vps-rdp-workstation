...
check "FR-11" "JetBrains Mono font installed" "fc-list | grep -qi jetbrains || find /home -name '*JetBrainsMono*' 2>/dev/null | grep -q . || test -d /usr/share/fonts/truetype/jetbrains-mono-nerd"
check "FR-12" "KDE Plasma installed" "dpkg -l | grep -E 'kde-plasma-desktop|plasma-workspace' | grep -q ii"
...
check "SR-6" "No plain-text passwords in logs" "! grep -rE 'password\s*=' /var/log/vps-setup*.log 2>/dev/null | grep -v 'password=ask'"
